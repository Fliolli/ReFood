...
class FoodsProvider implements BaseFoodsProvider {
  ...
  Future<List<GoodCardItem>> loadActiveGoods() async {
    ...
    if (activeGoods.isNotEmpty) {
      List<FoodModel> goods = await foods
          .where('id', whereIn: activeGoods)
          .get()
          .then(
              (value) => value.docs.map((e) => e.data() as FoodModel).toList());
      return goods
          .map((e) => GoodCardItem(e.id, e.image, e.name, e.price, e.unit,
              e.expirationDate, e.bookmarksCount, e.price == 0.0))
          .toList();
    }
    return [];
  }
    ...
    if (archivedGoods.isNotEmpty) {
      List<FoodModel> goods = await foods
          ...
          .then(
              (value) => value.docs.map((e) => e.data() as FoodModel).toList());
      return goods
          .map((e) => GoodCardItemTrimmed(e.id, e.image, e.name, e.price,
              e.unit, e.mass, e.mark, e.price == 0.0))
          .toList();
    }
    return [];
  }
  ...
  Future<List<OrderCardBookmarkItem>> loadBookmarkedOrders() async {
    var bookmarkedOrders =
        await global.userProvider.getBookmarkedOrdersItemID();
    if (bookmarkedOrders.isNotEmpty) {
      List<FoodModel> orders = await foods
          .where('id', whereIn: bookmarkedOrders)
          .get()
          .then(
              (value) => value.docs.map((e) => e.data() as FoodModel).toList());
      return Future.wait(orders
          .map((e) async => OrderCardBookmarkItem(
              e.id,
              e.image,
              e.name,
              e.price,
              e.unit,
              0.7,
              await global.userProvider.getUserTrimmed(e.ownerID),
              e.price == 0.0,
              e.ownerID))
          .toList());
    }
    ...
    if (archivedOrders.isNotEmpty) {
      List<FoodModel> orders = await foods
          ...
          .then(
              (value) => value.docs.map((e) => e.data() as FoodModel).toList());
      return Future.wait(orders
          .map((e) async => OrderCardItemTrimmed(
              e.id,
              e.image,
              e.name,
              e.price,
  ...
  Future<String> uploadFile(File imageFile) async {
    ...
    try {
      var path =
          'foodImages/${imageID + imageFile.path.substring(imageFile.path.lastIndexOf('.'))}';
